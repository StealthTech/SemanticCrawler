    ...
    def make(self, force=False):
        ...
        if not os.path.exists(self.path) or force:
            ...
            if self.markdown == 'main':
                payload = {
                    'PREDEFINED': [['secret_key'], {'debug': '0'}],
                    'DEPLOY': [{'allowed_hosts': 'localhost,127.0.0.1', 'static_root': 'static'}],
                    'DATABASES': [{'storage': 'mysql'}],
                    'MYSQL': [['name', 'user', 'password', 'tcp_addr', 'tcp_port']],
                    'REDIS': [['connection_type', 'socket_location', 'tcp_addr', 'tcp_port'], {'enabled': '0'}],
                    'EMAIL': [['host', 'host_user', 'host_password', 'port', 'default_from'],
                              {'enabled': '0', 'use_tls': '1'}],
                    'PICTURES': [{'avatar_max_height': '350', 'avatar_max_width': '350',
                                  'pic_max_height': '1000', 'pic_max_width': '1000'}],
                }
